...
export default function CaixaPage() {
  ...
  const handleOpenCash = () => {
    setLoading(true);
    const today = format(new Date(), "yyyy-MM-dd");
    cashSessionsStore.open(parseFloat(openingBalance) || 0, openingDate !== today ? openingDate : undefined);
  ...
  const handleCloseCash = () => {
    if (!currentSession) return;
    setLoading(true);
    cashSessionsStore.close(currentSession.id, {
  ...
  const handleSaveEntry = () => {
    ...
    try {
      if (editingEntry) {
        cashEntriesStore.update(editingEntry.id, {
      ...
      launchableAppts
        ...
        .forEach(appt => {
          ...
          if (appt.status !== "completed") {
            appointmentsStore.update(appt.id, { status: "completed", paymentStatus: "paid" });
          } else {
            appointmentsStore.update(appt.id, { paymentStatus: "paid" });
          }
